...
- (void)keyboardWillChange:(NSNotification *)notification {
    ...
    //bottom约束的大小(constant)
    self.bottomConstraint.constant = windowHeight - keyboardFrame.origin.y;
    ...
    if (aStream == self.inputStream) {
        ...
        while ([self.inputStream hasBytesAvailable]) {
            ...
            if (length > 0) {
                ...
                //table view滚动到哪里(作业part2)
                //获取数组中最新的(最后一项)那项对应的NSIndexPath
                NSIndexPath *lastIndexPath = [NSIndexPath indexPathForRow:self.messageArray.count - 1 inSection:0];
                //告诉table view滚动到的位置lastIndexPath
                [self.tableView scrollToRowAtIndexPath:lastIndexPath atScrollPosition:UITableViewScrollPositionBottom animated:YES];
            }
        }
    }
}
//在滚动视图滚动将要开始拖拽滚动的时候，取消view的第一响应
